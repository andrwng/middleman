...
func TestClassify(t *testing.T) {
	tests := []struct {
		...
		// Accuracy fix: a declaration keyword earlier on the line no
		// longer counts unless it is immediately adjacent to the
		// symbol. This is the reported bug -- "string" here used to
		// read as a definition just because "func" appears earlier on
		// the line -- paired with a check that the function's own
		// name on that exact same line is still correctly a
		// definition, since it IS adjacent to "func".
		{
			"a keyword earlier on the line, but not adjacent, is not a definition",
			"string", "func Get(key string) (string, bool) {",
			gitclone.KindReference,
		},
		{
			"the function's own name on that same line is still a definition",
			"Get", "func Get(key string) (string, bool) {",
			gitclone.KindDefinition,
		},

		// New shape: Go receiver methods. Adjacency alone misses these
		// -- the receiver clause's ")" sits between "func (...)" and
		// the method name -- so this is a structural check on the
		// whole line instead. The receiver's own type, on that same
		// line, stays a reference (its type definition is elsewhere).
		{
			"go receiver method",
			"Get", "func (c *Cache) Get(key string) (string, bool) {",
			gitclone.KindDefinition,
		},
		{
			"a go receiver method's receiver type is a reference, not a definition",
			"Cache", "func (c *Cache) Get(key string) (string, bool) {",
			gitclone.KindReference,
		},

		// New shape: C/C++ typed declarations and definitions -- an
		// identifier (the return type) immediately precedes the
		// symbol, which is immediately followed by "(". Also covers a
		// pointer/reference return type and storage/inline modifiers,
		// which must not break that adjacency.
		{
			"cpp typed function definition",
			"handle_request", "void handle_request(int fd) {",
			gitclone.KindDefinition,
		},
		{
			"cpp pointer-return function definition",
			"getCache", "Cache* getCache() {",
			gitclone.KindDefinition,
		},
		{
			"cpp reference-return function definition with storage/inline modifiers",
			"helper", "static inline Foo& helper() {",
			gitclone.KindDefinition,
		},

		// New shape: TS/JS export forms.
		{"ts export function", "run", "export function run() {", gitclone.KindDefinition},
		{"ts export const", "run", "export const run = () => {};", gitclone.KindDefinition},
		{"ts export class", "Widget", "export class Widget {", gitclone.KindDefinition},

		// Negatives: the C/C++ typed-declaration shape ("identifier
		// symbol(") also matches a plain call sitting right after a
		// control-flow or expression keyword. Each of these must stay
		// a reference, not get promoted to a definition.
		{"return statement is not a definition of the called function", "foo", "return foo(x);", gitclone.KindReference},
		{"if-statement call is not a definition", "cond", "if cond(...) {", gitclone.KindReference},
		{"new-expression is not a definition of the constructed type", "Thing", "new Thing(...);", gitclone.KindReference},
		{"throw-expression is not a definition of the thrown type", "Error", "throw Error(...);", gitclone.KindReference},
		{"sizeof is not a definition of its operand", "foo", "sizeof foo(...);", gitclone.KindReference},
		{"co_await is not a definition of the awaited call", "bar", "co_await bar(...);", gitclone.KindReference},
		{"case-label call is not a definition", "foo", "case foo(...):", gitclone.KindReference},
		{"delete-expression is not a definition of the deleted call", "p", "delete p(...);", gitclone.KindReference},
		// Not one of the task's literal examples, but the same class of
		// false positive the exclusion list guards against.
		{"assert call is not a definition", "foo", "assert foo(x) == 5;", gitclone.KindReference},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
